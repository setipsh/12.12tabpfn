...
def load_model_workflow(i, e, add_name, base_path, device='cpu', eval_addition='', only_inference=True):
    ...
    if e == -1:
        ...
        for e_ in [42] + list(range(143, -1, -1)):
...
def load_model_workflow_my(model_file, add_name, base_path, device='cpu', eval_addition='', only_inference=True):
    ...
    new_path = model_file.rsplit('.', 1)[0]+ eval_addition + '.pkl'
    ...
    def __init__(self, device='cpu', base_path=pathlib.Path(__file__).parent.parent.resolve(), model_string='',
        ...
        if model_file is None:
            ...
            if model_string in self.models_in_memory:
                ...
                model, c, results_file = load_model_workflow_my(i, 143, add_name=model_string, base_path=base_path, device=device,
            ...
            model, c, results_file = load_model_workflow_my(model_file = model_file,add_name=model_string, base_path=base_path, device=device,
                                                            eval_addition='', only_inference=only_inference)
        self.device = device
        self.model = model
        self.c = c
        ...
        self.model_file = model_file  # 在这里设置model_file属性
