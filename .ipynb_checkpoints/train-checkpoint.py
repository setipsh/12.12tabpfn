...
def train(priordataloader_class, criterion, encoder_generator, emsize=200, nhid=200, nlayers=6, nhead=2, dropout=0.0,
          ...
          initializer=None, initialize_with_model=None, train_mixed_precision=False, efficient_eval_masking=True, output_dim=None, **model_extra_args
          ):
    ...
    print(f'aggregate_k_gradients:{aggregate_k_gradients}')
    print(f'梯度下降的batch_size:{batch_size*aggregate_k_gradients}')
    print(f'num_datasets:{epochs*steps_per_epoch*batch_size}')
    print(f'train_mixed_precision:{train_mixed_precision}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--encoder', default='attention', type=str, help='Specify depending on the prior.')
    ...
    def get_encoder_generator(encoder):
        ...
        #修改111111111111111111111111111111111111111111111111111111
        elif encoder == 'attention':  # 添加对 ModelWithAttention 的支持
            encoder_generator = partial(encoders.ModelWithAttention, n_steps=3, gamma=1.3, epsilon=1e-15)
            print("Using ModelWithAttention as encoder_generator")
        else:
